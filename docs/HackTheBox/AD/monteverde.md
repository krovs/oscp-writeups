...
Get users with a null session via rdpclient and make a list
...
In users folder, the user mhope has a azure.xml file
...
Using the found credentials, evil-winrm to the machine
...
mhope belongs to Azure Admins group and the system has azure ad sync and a database installed so:
...
Searching an exploit we find https://github.com/VbScrub/AdSyncDecrypt, download the files and upload then to the machine and following the instructions, execute the exe.
...
Reenter as administrator via evil-winrm
