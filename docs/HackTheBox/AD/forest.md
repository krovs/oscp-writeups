...
Trying asrep-roasting attack we find a user with do not require kerbereros auth
...
The account svc-alfresco is member of Account Operators, this group can change users that don't belong to protected groups with high privileges, but we can find a usefull group.
...
Lastly, using admin's hash, enter the DC via evil-winrm.
