...
We enter the ftp with anonymous access and find a note and docx files
...
The note says to send a person a rtf file and he will convert it, so there is a person that is going to execute rtf files.

Using exiftool on the files we found an email.
...
So let's create a malicious rtf and send it to nico using the smtp server

Using searchsploit we get an interesting one
...
Now using sendEmail we send the yes.rtf to nico and create a listener on port 9090
...
So we get the pass in plain text using clixml
...
And tom has WriteOwner perms over claire so
...
Now we can go to administrator folder and there are multiple scripts in backup scripts fodler
...
Try administrator with this pass and
