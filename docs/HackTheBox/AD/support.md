...
UserInfo.exe seems like an internal program, let's see the interior with dotPeek
...
Put it in a cs file in a folder, install dot.net if not installed and
...
Looking at all the users whe see info field in support
...
Enumerating with bloodhound we see that our user has GenericAll over the dc host so we can perform a Resource-Based Constrained Delegation as seen here https://github.com/tothi/rbcd-attack
