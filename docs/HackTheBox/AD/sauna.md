...
There is an about page with the people of the company, we can make a userlist with the names.
...
We can try a ASREPRoasting with the user and blank password
...
Upload sharphound and `sh.exe -c All` and upload to bloodhound
...
Bloodhound tell us that tthis user can DCSync the domain so get hashes via secretsdump.
...
We have administrator hash, we can use evil-winrm with PTH
