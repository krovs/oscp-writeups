...
Clicking the tabs shows the full url and it worth to try a LFI/RFI
...
Web contains the two web pages and there is nothing in the rest.
...
s.moon can write into Shared folder, this seems like a client side attack so.
...
If we enumerate shares again, this is the senior dev, so he can now write the web folder.
...
There is a development folder in inetpub, looking at the listening ports, 8000 is open, so transfer chisel and  let's make a portforwarding to access the web page.
...
As this project runs on iis, let's upload a aspx reverse shell to pivot to another user.
...
We have seimpersonate priv with this one so:
...
Upload godpotato and a msfvenom reverse shell and:
