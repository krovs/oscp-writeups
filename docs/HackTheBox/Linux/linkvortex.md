...
Add linkvortex.htb to /etc/hosts and enter the webserver which is a hardware blog powered by Ghost service.

![](../assets/Pasted%20image%2020250506205857.png)

Nikto  find robots.txt with /ghost

![](../assets/Pasted%20image%2020250506210434.png)

Using wfuzz discover /dev subdomain and add it to /etc/hosts
...
With feroxbuster we find a .git so using git-dumpener
...
There is a LFI vulnerability https://github.com/TryGhost/Ghost/security/advisories/GHSA-9c9v-w225-v5rg so using an exploit https://github.com/0xyassine/CVE-2023-40028/blob/master/CVE-2023-40028.sh
...
If the link points to /etc or /root it gets deleted, if not, can be read if the variable CHECK_CONTENT is other than empty.
So first export the variable to true, then, create a link of a link, the script only the first link so.
...
Now the only thing is left is to access as root via the private key.
