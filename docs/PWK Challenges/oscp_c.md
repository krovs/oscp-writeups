...
```
eric.wallows:EricLikesRunning800 (given creds for 192.168.127.153)
web_svc:Diamond1
...
We search for mobile mouse server exploit

![](assets/Pasted%20image%2020250222004346.png)

We get  the script, I had to fix the breaklines; we need a http server to serve the rev shell made with msfvenom and a listener.
...
We have insecure permissions on a service, we can change the binary path for a malicious file and restart the service.

![](assets/Pasted%20image%2020250222013810.png)

Transfer the  revshell  and start a listener and:
...
Goin to 192.168.177.156:8083 we have the vesta panel login.
...
We can login as anonymous in the ftp. Inside, there is a bunch of pdf templates. Using exiftools we can get some usernames.
...
Start a nc listener and execute the reverse.
...
Making a pingsweep gets us the third machine

![](assets/Pasted%20image%2020250224004858.png)

In the erik folder we see a admintool.exe program, if we execute it and put a password, debug is enabled and shows the password comparisson in md5 hash. we can use hashcat to get it.
...
There is a windows.old on C: so we download sam and system using evil-winrm embeded download.
